 ...
 */
public class ZoneTimeRangeTest {

	private ZoneTimeRange target;
	...
	public void setUp() throws Exception {
		...
		target = new ZoneTimeRange(timeZone, start, end);

	}

	@Test(expected = NullPointerException.class)
	public void testTimeRange_NullTimeZone() {
		target = new ZoneTimeRange(null, start, end);
	}

	@Test(expected = NullPointerException.class)
	public void testTimeRange_NullStart() {
		target = new ZoneTimeRange(timeZone, null, end);
	}

	@Test(expected = NullPointerException.class)
	public void testTimeRange_NullEnd() {
		target = new ZoneTimeRange(timeZone, start, null);
	}
	...
	public void testEquals() {
		...
		assertTrue(target.equals(new ZoneTimeRange(timeZone, start, end)));
		assertFalse(target.equals(new ZoneTimeRange(tz, start, end)));
		assertFalse(target.equals(new ZoneTimeRange(timeZone, new Time(), end)));
		assertFalse(target
				.equals(new ZoneTimeRange(timeZone, start, new Time())));
	...
	public void testCompareTo() {
		...
		assertEquals(+0,
				target.compareTo(new ZoneTimeRange(timeZone, start, end)));
		assertEquals(+1, target.compareTo(new ZoneTimeRange(tz, start, end)));
		assertEquals(+1,
				target.compareTo(new ZoneTimeRange(timeZone, new Time(), end)));
		assertEquals(+1, target.compareTo(new ZoneTimeRange(timeZone, start,
				new Time())));
	}
	...
	public void testInRange_Default() throws Exception {
		...
		target = new ZoneTimeRange(timeZone, new Time(16, 0), new Time(3, 0));
